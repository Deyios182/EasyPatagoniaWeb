...
const TouristMapScreen: React.FC = () => {
  ...
  const googleMapsUrl = selectedBusiness 
    ? `https://maps.google.com/?q=${selectedBusiness.gps.lat},${selectedBusiness.gps.lng}`
    ...
    <div className="relative h-screen w-full bg-background-light dark:bg-background-dark overflow-hidden flex flex-col transition-colors duration-300">
      ...
      {showRouteAssistant && (
        <div className="fixed inset-0 z-[300] bg-black/50 backdrop-blur-xl flex items-end md:items-center justify-center p-4 animate-in fade-in duration-300 overflow-y-auto">
          <div className="bg-white dark:bg-surface-dark w-full max-w-3xl rounded-[3rem] overflow-hidden shadow-2xl border border-slate-200 dark:border-white/10 animate-in slide-in-from-bottom duration-500 my-auto">
            <div className="p-8 md:p-12 space-y-10">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-3xl font-black text-slate-800 dark:text-white uppercase tracking-tighter italic leading-tight">Guía de Aysén</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-primary/5 border-2 border-primary/20 p-8 rounded-[2.5rem] space-y-6">
                  ...
                  <div>
                    <h3 className="font-black text-2xl uppercase italic text-slate-800 dark:text-white leading-tight">Ir a {selectedBusiness?.nombre || 'Destino'}</h3>
                ...
                <div className="bg-slate-50 dark:bg-white/5 border-2 border-slate-200 dark:border-white/10 p-8 rounded-[2.5rem] space-y-6">
                  ...
                  <div>
                    <h3 className="font-black text-2xl uppercase italic text-slate-800 dark:text-white leading-tight">Vuelos a Balmaceda</h3>
                ...
                <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 scroll-smooth">
                  {localTransfers.map(op => (
                    <div 
                      ...
                      className="w-80 shrink-0 bg-slate-50 dark:bg-background-dark p-6 rounded-[2.5rem] border border-slate-200 dark:border-white/5 hover:border-primary transition-all cursor-pointer group shadow-sm flex flex-col h-auto"
                    >
                      <div className="flex items-center gap-4 mb-4">
                        <img src={op.media.logo_url} className="w-14 h-14 rounded-2xl object-cover shadow-md border border-white" />
                        <div className="min-w-0 flex-1">
                          <p className="text-lg font-black text-slate-800 dark:text-white uppercase italic leading-tight mb-1">{op.nombre}</p>
                          <p className="text-[9px] font-black text-primary uppercase tracking-widest leading-none">Verificado</p>
                        </div>
                      </div>
                      <p className="text-xs text-slate-600 dark:text-slate-400 font-medium mb-6 flex-1">
                        {op.info.descripcion}
                      </p>
                      <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-white/5">
                         <div className="flex items-center gap-1.5">
                            <span className="material-symbols-outlined text-primary text-sm leading-none">star</span>
                            <span className="text-sm font-black text-slate-700 dark:text-white leading-none">{op.rating}</span>
                         </div>
      ...
      <div className="absolute top-0 left-0 right-0 z-[100] p-6 md:p-8 flex flex-col md:flex-row gap-4 pointer-events-none">
        <div className="w-full md:max-w-md pointer-events-auto">
          {/* Fondo ajustado para modo claro y oscuro */}
          <div className="bg-white/90 dark:bg-surface-dark/90 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl px-8 py-5 flex items-center gap-4 shadow-2xl group focus-within:border-primary/50 transition-all">
            ...
            <input 
              ...
              className="bg-transparent border-none focus:ring-0 text-slate-800 dark:text-white placeholder-slate-400 w-full text-base font-bold py-1 leading-none" 
            />
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pointer-events-auto pb-2 scroll-smooth">
          {['All', 'Restaurante', 'Hospedaje', 'Actividad', 'Transporte'].map(cat => (
            <button 
              ...
              className={`whitespace-nowrap px-8 py-4 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border leading-none ${activeFilter === cat ? 'bg-primary border-primary text-white shadow-xl scale-105' : 'bg-white/80 dark:bg-surface-dark/80 text-slate-500 dark:text-slate-400 border-white/50 dark:border-white/5 backdrop-blur-md hover:bg-white dark:hover:bg-white/10'}`}
            >
      ...
      {/* TARJETA DE NEGOCIO SELECCIONADO Y BOTÓN CÓMO LLEGAR */}
      <div className="mt-auto relative z-[100] p-6 md:p-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6 pointer-events-none">
        {selectedBusiness && (
          <div 
            className="w-full md:max-w-md bg-white/90 dark:bg-surface-dark/95 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-[3rem] p-6 shadow-2xl flex items-center gap-6 animate-in slide-in-from-bottom duration-500 pointer-events-auto cursor-pointer group transition-colors duration-300" 
            onClick={() => navigate(`/details/${selectedBusiness.id}`)}
          >
            <div className="w-24 h-24 rounded-[2rem] overflow-hidden shrink-0 border border-slate-100 dark:border-white/5 relative">
            ...
            <div className="flex-1 min-w-0">
              ...
              <h3 className="text-slate-800 dark:text-white font-black text-2xl truncate leading-none tracking-tighter uppercase italic">{selectedBusiness.nombre}</h3>
              <div className="flex items-center gap-2 text-primary text-xs font-black mt-3 leading-none">
                ...
                <span className="text-slate-400 font-bold ml-1 uppercase tracking-widest text-[9px] font-bold leading-none">({selectedBusiness.reviewCount})</span>
              </div>
            </div>
            <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all shadow-xl">
        ...
        <div className="md:w-80 pointer-events-auto">
          <button 
            ...
            className="w-full bg-primary text-white font-black h-20 rounded-[2rem] shadow-2xl flex items-center justify-center gap-4 active:scale-95 transition-all uppercase tracking-[0.15em] text-xs shadow-primary/30 border-4 border-white/20 dark:border-white/10 px-6 py-4 leading-tight text-center"
          >
      ...
      <div className="md:hidden bg-white/95 dark:bg-surface-dark/95 backdrop-blur-2xl border-t border-slate-200 dark:border-white/5 p-6 pb-10 flex justify-around items-center z-[110]">
