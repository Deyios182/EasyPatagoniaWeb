            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
              ...
              <div className="relative group/locs flex-1 w-full max-w-full md:max-w-2xl">
                ...
                <div
                  ...
                  className="flex gap-3 overflow-x-auto no-scrollbar pb-2 px-2 touch-pan-x w-full snap-x snap-mandatory"
                >
                  {allLocalities.filter(l => l.is_active).map(loc => (
                    <button
                      key={loc.id}
                      onClick={() => setSelectedLocality(loc.id)}
                      className={`flex-none snap-center min-w-[70vw] md:min-w-0 px-6 py-3 rounded-full font-bold uppercase text-xs tracking-widest transition-all ${selectedLocality === loc.id ? 'bg-[#4f6d7a] text-white shadow-lg' : 'bg-white text-gray-400 border border-gray-200'}`}
                    >
                      {loc.name}
                    </button>
                  ))}
              ...
              <div className="lg:col-span-2 relative group-hover/section min-w-0 w-full">
                ...
                >
                  {visibleHighlights.length > 0 ? visibleHighlights.map(place => (
                    <div key={place.id} className="min-w-[85vw] md:min-w-[320px] group relative h-[500px] rounded-[3rem] overflow-hidden shadow-lg hover:shadow-2xl transition-all snap-center">
